...
响应式网关
  ...
  统一报文格式，提供RSA + AES报文混合加密，目前网关仅提供post请求和 文件上传类型的请求，拒绝get请求。
...
```
 ### 认证与权限
blink gateway整合了spring security 来实现登录认证和权限校验
目前实现了 base-app服务的管理用户用户名密码登录认证和权限校验。用户名密码校验通过后，通过UUID生成一个唯一Id作为用户token凭证
，将token存在redis中，并设置30分钟的过期时间。用户每个请求都会拦截获取token，与redis的做比对，token一致则请求通过，否则拒绝。
自动续期：如果用户则过期时间仍然剩10分钟时，仍然活跃 则将token过期时间延长30分钟.
这种方案是为了管理后台管理系统的用户状态，方便实现 踢人 强制下线等功能。

相关类：TokenServerAuthenticationConverter、 BlinkAuthorizationManager 、BlinkAuthenticationSuccessHandler

权限校验：是对RBAC权限管理模型的实现。通过redis缓存获取url对应的权限标识，校验用户登入成功时获取的权限集合，是否具有该标识，有则通过 无则拒绝。
 相关类：TokenAuthenticationManager

如果不追求管理用户登录状态，可以采用jwt 双token的方案（TODO）
 ### 多渠道对接管理
...
相关类：[RewriteRequestBodyFilter.java](src/main/java/com/blink/gateway/filter/RewriteRequestBodyFilter.java)
 ...
 #### 全局异常处理
 全局异常处理 这里只针对网关的错误进行错误信息组装，所有的错误都应该设置msg为错误码的形式，然后由全局异常处理统一根据错误码设置具体的错误信息。
 在blink系统中 采用HTTP 200 + 业务错误码的方案 业务上的错误码和http的状态码是分开设置，即业务上抛出错误，http状态码也是200.
...
相关类:[GlobalExceptionHandlerFilter.java](src/main/java/com/blink/gateway/filter/GlobalExceptionHandlerFilter.java)
 ...
 ### 灰度发布 
   通过动态路由已经可以实现一定程度的灰度发布。更细致的关于灰度发布的相关功能 暂时搁置 以后有空再弄
 **TODO**
 ### 性能参数监控
 **TODO**
### 其他功能
