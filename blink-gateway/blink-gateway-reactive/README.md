...
渠道的 `tokenType` 字段决定了网关采用哪种认证方式：

| tokenType | 常量名 | 认证方式 | 说明 |
|-----------|--------|----------|------|
| `-1` | `CHANNEL_AUTH_TYPE_CLOSE` | 用户Token认证 | 内部系统模式，不校验渠道token，使用用户登录后获取的token进行认证 |
| `0` | `CHANNEL_AUTH_TYPE_FIX_TOKEN` | 固定渠道Token认证 | TODO: 认证逻辑尚未实现 |
| `1` | `CHANNEL_AUTH_TYPE_JWT` | JWT渠道认证 | 渠道使用appKey、appSecret获取短期JWT，基于JWT进行认证 |
...
请求进入
   ↓
JwtAuthenticationFilter (处理 tokenType == 1 的渠道)
   ↓
TokenAuthenticationFilter (处理 tokenType == -1 的渠道)
   ↓
认证成功/失败处理
```

**注意**：如果渠道的 `tokenType` 配置为 `0`，当前两个过滤器都不会处理，会导致认证失败。请确保渠道配置正确的 `tokenType` 值。
...
[TokenAuthenticationManager](src/main/java/com/blink/gateway/security/token/TokenAuthenticationManager.java)、[TokenAuthenticationSuccessHandler](src/main/java/com/blink/gateway/security/token/TokenAuthenticationSuccessHandler.java)
...
| request_replay_defend_enable           | 防止请求重放开关       | false |
