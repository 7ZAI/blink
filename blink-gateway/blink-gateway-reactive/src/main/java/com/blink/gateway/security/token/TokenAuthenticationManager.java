...
import static com.blink.gateway.constant.RedisConstans.USER_TOKEN_OLD;
...
//        String loginName = (String) authentication.getPrincipal();
        ...
        return redisClient.get(tokenKey)
                .switchIfEmpty(Mono.defer(() -> {
                    // token 不存在，检查是否是被挤下线的 old token
                    String oldTokenKey = USER_TOKEN_OLD + token;
                    return redisClient.get(oldTokenKey)
                            .flatMap(userId -> {
                                // 存在 old token 标记，说明是被挤下线
                                return Mono.error(new CredentialsExpiredException("账号在其他设备登录，您已被挤下线"));
                            })
