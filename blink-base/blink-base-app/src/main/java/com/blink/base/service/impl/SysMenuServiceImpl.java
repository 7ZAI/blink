...
import com.blink.base.mapper.SysRoleMenuRelaMapper;
...
public class SysMenuServiceImpl implements SysMenuService {
    ...
    public SysMenuVO saveSysMenu(AddSysMenuReq saveParam) throws BlinkException {
        ...
        log.info("[SysMenu] 新增菜单成功 | menuId: {}, menuName: {}, type: {}, level: {}",
                sysMenuDO.getMenuId(), sysMenuDO.getMenuName(), menuType, menuLevel);
        return sysMenuVO;
    }
    ...
    public SysMenuVO modifySysMenu(UpdateSysMenuReq updateParam) throws BlinkException {
        ...
        //更换父节点
        if(ObjectUtil.isNotNull(updateParam.getParentId()) && !updateParam.getParentId().equals(oldParentId)){
            // 不能将自己设为父节点
            if (updateParam.getParentId().equals(updateParam.getMenuId())) {
                BlinkException.throwBusinessException(BaseErrCodeConstant.PARAMETER_OUT_RANGE);
            }
    ...
    public QueryShowMenuRsp getAllMenus() throws BlinkException {
        //查询所有菜单（不限制角色）
        List<SysMenuDO> allMenus = sysMenuMapper.selectList(null);
