...
const visitedViews = computed(() => tabsStore.getTabs)

const isActive = (tag: TagItem) => tag.path === route.path
...
const getTabTitle = (tag: TagItem): string => {
...
const closeSelectedTag = (tag: TagItem) => {
  const nextPath = tabsStore.closeTab(tag.path)
  if (nextPath) {
...
const openContextMenu = (e: MouseEvent, tag: TagItem) => {
  e.preventDefault()
  e.stopPropagation()
...
const handleContextCommand = (command: string) => {
  ...
  switch (command) {
    ...
    case 'close':
      ...
      break
    case 'closeOthers':
      closeOtherTags(selectedTag.value)
      break
    case 'closeRight':
...
const refreshTag = (tag: TagItem) => {
  ...
  tabsStore.delCachedView(tag.name)
  
  nextTick(() => {
    tabsStore.addCachedView(tag.name!)
    if (tag.path === route.path) {
      router.replace({
...
const closeOtherTags = (tag: TagItem) => {
  tabsStore.closeOtherTabs(tag.path)
...
const closeRightTags = (tag: TagItem) => {
  const tagIndex = visitedViews.value.findIndex((t) => t.path === tag.path)
  const rightTags = visitedViews.value.slice(tagIndex + 1)
  rightTags.forEach((t) => {
    if (!t.affix) {
      tabsStore.closeTab(t.path)
    }
  })
  if (route.path !== tag.path && !visitedViews.value.find(t => t.path === route.path)) {
    router.push(tag.path)
  }
  ElMessage.success(t('message.success'))
}

const closeLeftTags = (tag: TagItem) => {
  const tagIndex = visitedViews.value.findIndex((t) => t.path === tag.path)
  const leftTags = visitedViews.value.slice(0, tagIndex)
  leftTags.forEach((t) => {
    if (!t.affix) {
      tabsStore.closeTab(t.path)
    }
...
const closeAllTags = () => {
  ...
  const remainingTabs = tabsStore.getTabs
