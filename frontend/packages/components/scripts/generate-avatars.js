...
async function generateAvatars() {
  ...
  for (const style of avatarStyles) {
    try {
      ...
      const filePath = path.join(outputDir, `${style.name}.svg`)

      fs.writeFileSync(filePath, svg)
      console.log(`✓ 生成: ${style.name}.svg`)
    } catch (error) {
      console.error(`✗ 失败: ${style.name} - ${error.message}`)
    }
  }

  console.log(`\n头像生成完成！保存在: ${outputDir}`)
  console.log(`共生成 ${avatarStyles.length} 个头像`)
}
