    ...
    <el-card class="table-card" shadow="never">
      ...
      <div class="table-wrapper">
        ...
        >
          ...
          <el-table-column prop="roleType" :label="t('role.roleType')" width="100" align="center">
            <template #default="{ row }">
              <el-tag v-if="row.roleType === 1" type="primary">{{ t('role.typeSystem') }}</el-tag>
...
const handleDelete = async (row: RoleInfo) => {
  try {
    await ElMessageBox.confirm(t('role.deleteConfirm'), t('message.tips'), {
      type: 'warning',
    })
    await deleteRole({
      deleteId: row.roleId,
...
/* 角色管理页面样式 - 继承全局 table-page-container 样式 */
.role-management {
  /* 可添加页面特定的样式覆盖 */
}
</style>
